#!/usr/bin/env ruby
# encoding: utf-8
# Global variables and helper methods for SUSHI applications
# Adapted from old SUSHI system for compatibility
...
module GlobalVariables
  ...
  def ref_selector(value_replace_pattern=GENOME_REF_DIRS.inject({}){|hash, dir| hash[dir+"/"]=''; hash}, cache_name='ref_selector')
    selector = {'select'=>''}
    # Use Rails cache if available, otherwise just compute
    if defined?(Rails) && Rails.respond_to?(:cache)
      selector = Rails.cache.fetch(cache_name, expired_in: 1.hour) do
        build_ref_selector(value_replace_pattern)
      end
    else
      selector = build_ref_selector(value_replace_pattern)
    end
    selector
  end

  def build_ref_selector(value_replace_pattern)
    selector = {'select'=>''}
  ...
  def run_RApp(app_name = self.class.to_s[0].downcase+self.class.to_s[1,20], lib_path: nil, conda_env: nil)
    ...
    grandchild_data = respond_to?(:grandchild_datasets) ? grandchild_datasets : []
