...
# (app/controllers/v1/datasets_controller.rb). Ported from legacy production
# SUSHI (uzh/sushi).
#
...
#
# key_value is written via Sample.serialize_key_value_ruby (the legacy Ruby
# Hash#inspect format) so btools / legacy SUSHI can eval() it back. This is the
# hardened equivalent of the legacy `sample_hash.to_s` (identical on Ruby 3.3.7;
# version-independent on 3.4+ where Hash#to_s adds spaces around =>).
#
...
class DatasetRegistrationService
  ...
  # Composed, caller-context idempotency key. The same content under a different
  # project/name/parent is a DIFFERENT registration.
  ...
  # User (created on first use). Static tokens pass nil (no person).
  def self.register(dataset_tsv:, project_number:, name:, parent_id: nil, order_id: nil, owner_login: nil)
    ...
    begin
      ...
      ActiveRecord::Base.transaction do
        ...
        # Owner ("Who"): set to an EXISTING user matching the token login. Unlike
        # legacy SUSHI (LDAP-only users table), New SUSHI users are Devise-backed
        # with NOT NULL email/password, so we do not auto-provision an LDAP owner
        # here; an unknown login leaves the dataset unowned (UI fallback). Auto-
        # provisioning LDAP owner users is a follow-up tied to the users-model
        # reconciliation.
        data_set.user = User.find_by(login: owner_login) if owner_login.present?

        rows.each do |row|
          sample_hash = {}
          headers.each_with_index { |header, i| sample_hash[header] = row[i] }
          # Legacy Ruby Hash#inspect format (hardened; see class comment).
          data_set.samples.build(key_value: Sample.serialize_key_value_ruby(sample_hash))
        end
      ...
      # Concurrent duplicate won the unique (registration_key) race → treat as replay.
      existing = DataSet.find_by(registration_key: key)
      if existing
